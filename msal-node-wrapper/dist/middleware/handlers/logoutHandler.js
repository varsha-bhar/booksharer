...
function logoutHandler(options) {
    return async (req, res) => {
        ...
        if (shouldLogoutFromIdp) {
             ...
             */
            const postLogoutRedirectUri = UrlUtils.UrlUtils.ensureAbsoluteUrl(options.postLogoutRedirectUri || "/", req.protocol, req.get("host") || req.hostname);
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            logoutUri = `${UrlUtils.UrlUtils.enforceTrailingSlash(this.getMsalConfig().auth.authority)}/oauth2/v2.0/logout?post_logout_redirect_uri=${postLogoutRedirectUri}`;
        }
        req.session.destroy(() => {
